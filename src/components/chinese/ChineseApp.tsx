...
// hoạt động), cùng pattern với /ielts/<skill>/lessons/<id> thay vì chỉ đổi state trong trang.
function pathForSelection(s: Selection): string {
  switch (s.type) {
    case 'lesson':
      return `/chinese/lessons/${s.lesson}`
    case 'phonetics':
      return `/chinese/phonetics/${s.lesson}`
    case 'deck':
      return `/chinese/decks/${s.deckId}`
    default:
...
export function ChineseApp() {
  ...
  const navigatingTo = useChineseUIStore((s) => s.navigatingTo)
  const beginNavigating = useChineseUIStore((s) => s.beginNavigating)
  const endNavigating = useChineseUIStore((s) => s.endNavigating)

  const router = useRouter()
  const pathname = usePathname()
  const params = useParams<{ lesson?: string; deckId?: string }>()
  ...
  const selection: Selection = pathname.startsWith('/chinese/lessons/')
    ? { type: 'lesson', lesson: Number(params.lesson) }
    : pathname === '/chinese/lessons'
      ? { type: 'knowledge' }
    : pathname.startsWith('/chinese/phonetics/')
      ? { type: 'phonetics', lesson: Number(params.lesson) }
      : pathname.startsWith('/chinese/decks/')
        ? { type: 'deck', deckId: String(params.deckId) }
        : { type: 'overview' }
  ...
  const [kindFilter, setKindFilter] = useState<'all' | ChineseCardKind>(contentKind ?? 'all')
  ...
  // bằng query string (?open=hsk3), đọc 1 lần lúc mount (xem prop initialGroup của Sidebar).
  const initialGroup = (searchParams.get('open') as HskLevel | 'phonetics' | null) ?? null
  ...
  const overviewLevel = HSK_LEVELS.find(({ key }) => key === initialGroup)?.key
  const overviewCards = overviewLevel ? sortedCards.filter((card) => LESSON_META[card.lesson]?.level === overviewLevel) : sortedCards
  const overviewLearnedCount = overviewCards.filter((card) => isLearned(card.id)).length
  const overviewLearnedPercent = overviewCards.length > 0 ? Math.round((overviewLearnedCount / overviewCards.length) * 100) : 0
  ...
  function handleDeleteDeck(deckId: string, deckName: string) {
    ...
    deleteDeck(deckId)
    if (selection.type === 'deck' && selection.deckId === deckId) navigate('/chinese/vocab')
  }
    ...
    <div className="cn-shell">
      {!showLanding && (
        <Sidebar
          ...
          onOpenKnowledge={(group) => navigate(`/chinese/lessons?open=${group}`)}
      ...
      <div className="cn-main">
        ...
        {showLanding ? (
          ...
          <KnowledgeIndex
            ...
            title={initialGroup === 'phonetics' ? 'Kiến thức Ngữ âm cơ bản' : `Kiến thức ${HSK_LEVELS.find((level) => level.key === (initialGroup ?? 'hsk12'))?.label ?? 'HSK 1+2'}`}
            subtitle={`${initialGroup === 'phonetics' ? PHONETICS_LESSONS.length : lessonNumbersForLevel((initialGroup as HskLevel) ?? 'hsk12').length} bài học · chọn một bài để bắt đầu`}
            items={(initialGroup === 'phonetics' ? PHONETICS_LESSONS.map((lesson) => ({ number: lesson.number, title: lesson.title })) : lessonNumbersForLevel((initialGroup as HskLevel) ?? 'hsk12').map((lesson) => {
            ...
            onPick={(lesson) => navigate(initialGroup === 'phonetics' ? `/chinese/phonetics/${lesson}` : `/chinese/lessons/${lesson}`)}
          />
    ...
    <div className="cn-content">
      <div className="cn-content-header">
        <div>
          ...
          <h1 className="cn-page-title">{contentKind === 'grammar' ? 'Tất cả ngữ pháp' : 'Tất cả từ vựng'}</h1>
        </div>
