...
// Cỡ chữ co theo độ dài để mẫu ngữ pháp dài (VD "N(이)라고 하다") không bị tràn khung.
function frontSizeClass(length: number) {
...
// ra sẽ bay thẻ ra khỏi màn hình và báo kết quả, giống thao tác quẹt Tinder. Cùng convention với
// components/chinese/FlashCard.tsx.
export function FlashCard({ kind, front, meaning, note, example, flipped, onFlip, onSwipe }: FlashCardProps) {
      ...
      >
        ...
        >
          ...
          <div className="flex h-full flex-col items-center justify-center gap-3 overflow-y-auto p-6 text-center">
            <span className={`font-bold leading-tight text-accent-strong ${meaningSizeClass(meaning.length)}`}>{meaning}</span>
            {note && <span className="rounded-pill bg-white/70 px-3 py-1 text-sm font-medium text-muted">{note}</span>}
            {example && <span className="whitespace-pre-line text-xs text-muted">{example}</span>}
            <span className="absolute bottom-6 text-xs text-muted">👆 Chạm xem lại · 👉 Quẹt chấm điểm</span>
          </div>
        </div>
      </div>
