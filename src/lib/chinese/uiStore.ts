...
interface ChineseUIState {
  ...
  openAddCard: () => void
  closeAddCard: () => void
  // Nhóm cấp độ đang mở trong Sidebar (HSK1, "Ngữ âm cơ bản"…) — CHỈ 1 nhóm mở tại 1 thời điểm (kiểu
  // accordion, giống Kiến thức/Làm đề/Vocab bên IELTS không bao giờ mở chồng lên nhau) để menu không bị
  // dài loằng ngoằng khi mở nhiều nhóm cùng lúc. Sống ở store module-level (không reset khi component
  // unmount) để KHÔNG bị đóng lại mỗi lần chuyển bài (Sidebar giờ remount theo route, xem
  // components/chinese/ChineseApp.tsx). Vẫn thu gọn khi F5/vào lại trang vì store không persist ra
